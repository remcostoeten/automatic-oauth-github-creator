...
class GitHubAutomator:
    ...
    def ensure_logged_in(self) -> bool:
        ...
        if "/login" in self.page.url or self.page.query_selector(GitHubSelectors.LOGIN_INPUT):
            ...
            try:
                ...
                self.page.wait_for_selector(GitHubSelectors.LOGGED_IN_META, timeout=300_000) # 5 min timeout
    ...
    def handle_sudo_mode(self):
        ...
        if is_passkey:
            ...
            if not clicked:
                logger.warning("   Could not find 'Use your password' link, please click it manually")
    ...
    def _generate_secret(self):
        ...
        if not clicked:
            ...
            for tag in ['button', 'a', 'input', 'summary']:
                for el in self.page.query_selector_all(tag):
                    try:
                        text = el.inner_text().lower() if tag != 'input' else (el.get_attribute('value') or '').lower()
        ...
        if not clicked:
            raise Exception("Could not find 'Generate client secret' button. Please click it manually!")
...
def interactive_create():
    ...
    default_callback = os.getenv("OAUTH_CALLBACK_URL", f"{default_homepage}/api/auth/callback/github")
...
def view_saved_credentials():
    ...
    import re
    matches = re.findall(r'# GitHub OAuth Credentials.*?\nGITHUB_CLIENT_ID="([^"]+)"\nGITHUB_CLIENT_SECRET="([^"]+)"', content)
    ...
    if prompt_yes_no("\nTest a saved credential?", False):
        try:
            choice = int(input("\033[94m➤\033[0m Enter credential number to test: ").strip())
...
def interactive_delete():
    ...
    try:
        ...
        print()
        print("\033[90mTip: You can enter multiple numbers separated by commas (e.g. '1, 3, 5')\033[0m")
        
        try:
            choice_str = input("\033[94m➤\033[0m Enter app number(s) to delete (0 to cancel): ").strip()
            ...
            # e.g. "1, 3, 5" -> [1, 3, 5]
            try:
                choices = [int(c.strip()) for c in choice_str.split(',') if c.strip().isdigit()]
            except ValueError:
...
def main():
    ...
    parser.add_argument("--app-name", required=True, help="Name of the OAuth application")
    parser.add_argument("--password", "-p", help="GitHub password for sudo mode authentication")
    parser.add_argument("--write-env", action="store_true", help="Append credentials to .env file")
    parser.add_argument("--verify", "-v", action="store_true", help="Verify credentials after creation")
    parser.add_argument("--homepage-url", default="http://localhost:3000", help="Homepage URL")
    parser.add_argument("--callback-url", help="Callback URL (defaults to homepage + /api/auth/callback/github)")
